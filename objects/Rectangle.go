...
func (p *Rectangle) Draw(renderer *sdl.Renderer) {
	if p.Enabled {
		if p.TextureData != nil {
			renderer.CopyEx(p.TextureData.Texture, p.TextureData.Rect, p.Rect(), -p.RotationAccu, &sdl.Point{X: p.W / 2, Y: p.H / 2}, sdl.FLIP_NONE)
		} else {
			xo := p.XOrigin
			yo := p.YOrigin
			if p.Filled {
...
func NewRectangle(name string, px1, py1, px2, py2, px3, py3, px4, py4, pxOrigin, pyOrigin float64, col sdl.Color, enabled bool, filled bool) *Rectangle {
